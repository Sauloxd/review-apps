"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.onDefault = void 0;
var io = require("@actions/io");
var core = require("@actions/core");
var exec_1 = require("@actions/exec");
var manifest = require("../utils/manifest");
var retry_1 = require("../utils/retry");
var git = require("../utils/git");
var fileManager = require("../utils/file-manager");
var user_input_1 = require("../utils/user-input");
function onDefault(params) {
    return __awaiter(this, void 0, void 0, function () {
        var input, paths;
        var _this = this;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    input = (0, user_input_1.userInput)();
                    paths = fileManager.paths(params);
                    core.info("\n    -> Paths:\n    -> Your app will be hosted in github pages:\n    -> \"https://{ username }.github.io/{ repository }/{ slug }/{ branch }/{ head_commit }\"\n    -> This app URL:\n    -> @TODO\n\n    -> Example:\n    -> \"https://sauloxd.github.io/review-apps/storybook/feature-1/c1fcf15\"\n\n    -> We'll build your app with the proper PUBLIC_URL\n    -> For more info:\n    -> https://github.com/facebook/create-react-app/pull/937/files#diff-9b26877ecf8d15b7987c96e5a17502f6\n    -> https://www.gatsbyjs.com/docs/path-prefix/\n  ");
                    core.info("\n    -> Building app\n  ");
                    core.exportVariable('PUBLIC_URL', "/".concat(paths.byRepo, "/").concat(paths.byHeadCommit));
                    return [4 /*yield*/, (0, exec_1.exec)(input.buildCmd)];
                case 1:
                    _a.sent();
                    core.info("\n    -> Current working branch: ".concat(params.branch.name, "\"\n    -> Will move (and override) the build result on '").concat(input.dist, "' to '").concat(paths.byHeadCommit, "' in ").concat(input.branch, "\"\n  "));
                    git.stageChanges(input.dist);
                    git.commit("Persisting dist output for ".concat(input.slug));
                    return [4 /*yield*/, (0, retry_1.retry)(5)(function () { return __awaiter(_this, void 0, void 0, function () {
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0:
                                        git.hardReset(input.branch);
                                        git.getFilesFromOtherBranch(params.branch.name, input.dist);
                                        manifest.replaceApp(params);
                                        return [4 /*yield*/, (0, exec_1.exec)('git', ['status'])];
                                    case 1:
                                        _a.sent();
                                        core.debug('Coping from input.dist to -> ' + paths.byHeadCommit);
                                        return [4 /*yield*/, io.cp(input.dist, paths.byHeadCommit, {
                                                recursive: true,
                                                force: true
                                            })];
                                    case 2:
                                        _a.sent();
                                        return [4 /*yield*/, (0, exec_1.exec)('git', ['status'])];
                                    case 3:
                                        _a.sent();
                                        git.stageChanges(paths.byHeadCommit, 'index.html', 'manifest.json');
                                        git.commit("Updating app ".concat(params.branch.headCommit));
                                        git.push(params.branch.name);
                                        return [2 /*return*/];
                                }
                            });
                        }); })];
                case 2:
                    _a.sent();
                    core.debug('Return to original state');
                    git.hardReset(params.branch.name);
                    return [2 /*return*/];
            }
        });
    });
}
exports.onDefault = onDefault;
